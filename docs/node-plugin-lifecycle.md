...
sequenceDiagram
    ...
    Note over WEB: 不参与 Node Named Pipe 启动

    BH->>SM: StartSessionAsync()
    SM->>PC: StartAsync(pipeName, pluginId)
    PC->>PC: 创建 Pipe、启动 Node
    PC-->>SDK: stdin: pipePath
...
1. 宿主创建 Named Pipe、启动 Node，并通过 stdin 发送管道路径。
2. Node 连接管道后即可收发业务消息，不再发送 `bus.handshake`。
3. 协议版本已在启动前通过 `plugin.json` 校验。
4. 宿主把 transport 绑定到 `pluginId`、`sessionId`、`endpointId`，注册总线端点并将会话置为 `Ready`。envelope 不携带身份字段，来源完全由这一 transport 绑定决定。

会话状态为：`Created → Starting → Ready`。连接失败或超时会停止该 Node 进程。
...
- 若会话尚未启动，首次发送初始化请求会先完成上面的启动与管道连接。
...
- 自动重启会创建新的 pipe 和 `sessionId`；旧请求失败。重启采用指数退避，并限制 5 分钟内最多 2 次；新会话下次使用时会重新初始化。
...
3. **缺少整体启动超时**：Node 若一直不连接 Pipe，启动可以无限等待。

### 中优先级

5. **`Ready` 不等于业务可用**：Session 在管道连上后即为 `Ready`，插件的 `plugin.call.initialize` 尚未完成；自动重启后也不会立即重新初始化。
...
9. `PluginLoader` 会并发初始化所有启用插件，插件较多时可能同时拉起大量 Node 进程。

## 关键实现

- 宿主启动：`MyTools.Host.Core/Sessions/PluginSessionManager.cs`
