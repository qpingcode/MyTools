...
function toSearchItem(item: BrowserItem, query: string, index: number) {
  ...
  return {
    id: `bookmark-and-history:${item.browser}:${item.kind}:${index}:${item.url}`,
...
async function loadBrowserSafely(
  ...
  try {
    ...
    console.error(`[bookmark-and-history] ${name} failed: ${message}`);
    return [];
  }
}
...
async function search(params: PluginSearchParams) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    console.error(`[bookmark-and-history] search failed: ${message}`);
    return { items: [] };
  }
  const matched = items
