...
export function readChromiumBookmarksFile(filePath: string, browser: BrowserKind, profileName: string): BrowserItem[] {
  ...
  try {
    ...
    console.error(`[bookmark-and-history] chromium bookmarks failed for ${filePath}: ${message}`);
    return [];
  }
}
  ...
  return cachedAsync(
    ...
    async () => {
      try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        console.error(`[bookmark-and-history] chromium history failed for ${filePath}: ${message}`);
        return [];
      }
    },
  );
...
export async function loadChromiumItems(
  ...
  for (const profile of profiles) {
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      console.error(`[bookmark-and-history] ${browser} profile ${profile.name} failed: ${message}`);
    }
  }
  return items;
}
