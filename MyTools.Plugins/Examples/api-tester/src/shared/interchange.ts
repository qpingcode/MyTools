...
export function curlTokens(source: string): string[] {
    ...
    for (let index = 0; index < source.length; index++) {
        ...
        if (quote) {
            ...
            if (++index >= source.length) throw new Error('Incomplete escape');
            token += source[index];
            started = true;
        } else {
            token += char;
...
export function importCurl(source: string, id: string, name: string): ApiRequest {
    const args = curlTokens(source);
    const r = newRequest(id, name);
    ...
    for (let index = 0; index < args.length; index++) {
        ...
        if (flag.startsWith('--') && flag.includes('=')) {
            ...
            inline = flag.slice(split + 1);
            flag = flag.slice(0, split);
        } else if (/^-[XHubdFAe]/.test(flag) && flag.length > 2) {
            inline = flag.slice(2);
            flag = flag.slice(0, 2);
        }
        const value = () => {
            const v = inline ?? args[++index];
            if (v === undefined) throw new Error(`Missing ${flag}`);
            return v;
        };
        if (/^https?:\/\//i.test(flag)) {
            if (r.url) throw new Error('Multiple URLs');
            r.url = flag;
        } else switch (flag) {
            case '--url':
                r.url = value();
            ...
            case '--header': {
                ...
                if (split <= 0) throw new Error('Invalid header');
            ...
            case '--cookie': {
                const cookie = value();
                if (!cookie.includes('=')) throw new Error('Cookie files are unsupported');
                r.headers.push(pair(HttpHeader.Cookie, cookie));
                break;
            }
            case '-A':
            case '--user-agent':
                r.headers.push(pair(HttpHeader.UserAgent, value()));
                break;
            case '-e':
            case '--referer':
                r.headers.push(pair(HttpHeader.Referer, value()));
                break;
            case '-u':
            case '--user': {
                const credentials = value();
                const split = credentials.indexOf(':');
                r.auth.kind = AuthKind.Basic;
                r.auth.username = split < 0 ? credentials : credentials.slice(0, split);
                r.auth.password = split < 0 ? '' : credentials.slice(split + 1);
                break;
            }
            case '-d':
            case '--data':
            ...
            case '--json': {
                const body = value();
                if (body.startsWith('@') && flag === '--data-binary') {
                    if (body === '@-' || r.body.kind !== BodyKind.None) throw new Error('Standard input or mixed files are unsupported');
                    r.body.kind = BodyKind.Binary;
                    r.body.file = body.slice(1);
                    r.body.contentType = ContentType.Form;
                    break;
                }
                if (body.startsWith('@') && flag !== '--data-raw') throw new Error('File-backed data is unsupported');
                data.push(body);
                json ||= flag === '--json';
                break;
            }
            case '--data-urlencode': {
                ...
                if (field.includes('@') && (split < 0 || field.indexOf('@') < split)) throw new Error('File-backed data is unsupported');
                data.push(split < 0 ? encodeURIComponent(field) : field.slice(0, split) + '=' + encodeURIComponent(field.slice(split + 1)));
                form = true;
                break;
            }
            case '-F':
            ...
            case '--form-string': {
                ...
                const field = value();
                const split = field.indexOf('=');
                if (split < 0) throw new Error('Invalid form field');
                r.body.kind = BodyKind.Multipart;
                const p = pair(field.slice(0, split), field.slice(split + 1));
                if (p.value.startsWith('@') && flag !== '--form-string') {
                    const [file, type] = p.value.slice(1).split(';type=');
                    p.file = file;
                    p.value = '';
                    p.contentType = type || ContentType.Binary;
                }
                r.body.fields.push(p);
                break;
            }
            case '-k':
            case '--insecure':
                r.settings = {...emptyWorkspace().defaults, ...r.settings, verifyTls: false};
                break;
            case '-L':
            case '--location':
                r.settings = {...emptyWorkspace().defaults, ...r.settings, followRedirects: true};
                break;
            case '--max-time': {
                const timeoutMs = Number(value()) * MillisecondsPerSecond;
                if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) throw new Error('Invalid timeout');
                r.settings = {...emptyWorkspace().defaults, ...r.settings, timeoutMs};
                break;
            }
            case '--compressed':
            case '-s':
    ...
    if (!['http:', 'https:'].includes(address.protocol)) throw new Error('Invalid URL');
    ...
    if (data.length) {
        ...
        r.body.contentType = type;
        r.body.text = data.join('&');
        r.body.kind = form ? BodyKind.Form : type.includes('json') ? BodyKind.Json : BodyKind.Text;
...
export function resolveRequestVariables(request: ApiRequest, replace: (text: string) => string): ApiRequest {
    ...
    r.url = replace(r.url);
    const resolvePairs = (pairs: Pair[]) => pairs.map(p => !p.enabled ? p : {
        ...p,
        name: replace(p.name),
        value: p.file ? p.value : replace(p.value),
        file: p.file ? replace(p.file) : undefined
    });
    r.params = resolvePairs(r.params);
    r.headers = resolvePairs(r.headers);
...
export function exportCurl(request: ApiRequest): string {
    const r = clone(request);
    const args = ['curl', '--request', shellQuote(r.method), shellQuote(queryUrl(r.url, r.params))];
    for (const h of r.headers.filter(h => h.enabled)) args.push('--header', shellQuote(h.name + ': ' + h.value));
    switch (r.auth.kind) {
        case AuthKind.Basic:
            args.push('--user', shellQuote(r.auth.username + ':' + r.auth.password));
            break;
        case AuthKind.Bearer:
            args.push('--header', shellQuote('Authorization: Bearer ' + r.auth.token));
            break;
    ...
    const contentHeader = () => {
        if (!r.headers.some(h => h.enabled && h.name.toLowerCase() === HttpHeader.ContentType)) args.push('--header', shellQuote('Content-Type: ' + (r.body.contentType || (r.body.kind === BodyKind.Json ? ContentType.Json : r.body.kind === BodyKind.Form ? ContentType.Form : r.body.kind === BodyKind.Binary ? ContentType.Binary : ContentType.Text))));
    };
    if ([BodyKind.Json, BodyKind.Text].includes(r.body.kind)) {
        contentHeader();
        args.push('--data-raw', shellQuote(r.body.text));
    }
    if (r.body.kind === BodyKind.Form) {
        contentHeader();
        args.push('--data-raw', shellQuote(queryUrl('', r.body.fields).slice(1)));
    }
    ...
    if (r.body.kind === BodyKind.Binary) {
        contentHeader();
        args.push('--data-binary', shellQuote('@' + r.body.file));
    }
...
function postmanAuth(source: any): ApiRequest['auth'] {
    ...
    switch (source.type) {
        ...
        case 'apikey':
            ...
            auth.key = fields.key || '';
            auth.value = fields.value || '';
            auth.location = fields.in === 'query' ? KeyLocation.Query : KeyLocation.Header;
            break;
        default:
            throw new Error(`Unsupported Postman auth: ${source.type}`);
    }
    return auth;
}
...
export function importWorkspace(source: string, uid: () => string): Workspace {
    ...
    if (parsed.version !== undefined) {
        ...
        Object.assign(workspace, clone(parsed));
        ...
        const collection: Collection = {
            ...
            requests: [],
            headers: [],
            auth: postmanAuth(parsed.auth),
            scripts: postmanScripts(parsed.event)
        };
        ...
        const visit = (items: any[], prefix: string, inheritedAuth: any, inheritedEvents: any[]) => {
            for (const item of items) {
                if (item.item) {
                    visit(item.item, prefix + String(item.name || '') + ' / ', item.auth || inheritedAuth, [...inheritedEvents, ...(item.event || [])]);
                    continue;
                }
                ...
                if (body?.mode === 'raw') {
                    r.body.kind = body.options?.raw?.language === 'json' ? BodyKind.Json : BodyKind.Text;
                    r.body.text = String(body.raw || '');
                } else if (body?.mode === 'urlencoded' || body?.mode === 'formdata') {
                    r.body.kind = body.mode === 'urlencoded' ? BodyKind.Form : BodyKind.Multipart;
                    r.body.fields = (body[body.mode] || []).map((p: any) => ({
                        ...pair(String(p.key || ''), String(p.value || ''), !p.disabled), ...(p.type === 'file' ? {
                            file: String(Array.isArray(p.src) ? p.src[0] || '' : p.src || ''),
                            contentType: p.contentType || ContentType.Binary
                        } : {})
                    }));
                } else if (body?.mode === 'file') {
                    r.body.kind = BodyKind.Binary;
                    r.body.file = String(body.file?.src || '');
                } else if (body?.mode && body.mode !== 'raw') throw new Error(`Unsupported Postman body: ${body.mode}`);
                collection.requests.push(r);
            }
        };
        visit(parsed.item, '', undefined, []);
        workspace.collections.push(collection);
        if (Array.isArray(parsed.variable) && parsed.variable.length) workspace.environments.push({
            id: uid(),
            name: collection.name,
            variables: parsed.variable.map((v: any) => pair(String(v.key || ''), String(v.value || ''), !v.disabled))
        });
    } else if (Array.isArray(parsed.values)) {
        workspace.environments.push({
            id: uid(),
            name: String(parsed.name || ''),
            variables: parsed.values.map((v: any) => pair(String(v.key || ''), String(v.value ?? ''), v.enabled !== false))
        });
    ...
    for (const c of workspace.collections) {
        ...
        for (const r of c.requests) {
            r.id = uid();
            if (r.scripts) r.scripts.enabled = false;
        }
    }
    for (const e of workspace.environments) {
        const old = e.id;
        e.id = uid();
        if (old === activeEnvironment) workspace.environmentId = e.id;
    }
...
export function exportWorkspace(workspace: Workspace, collection?: Collection, environment?: Environment): string {
    ...
    if (environment) {
        result.collections = [];
        result.environments = [clone(environment)];
...
export function exportPostman(collection: Collection): string {
    const auth = (a?: ApiRequest['auth']): any => {
        ...
        switch (a.kind) {
            case AuthKind.None:
                return {type: 'noauth'};
            case AuthKind.Basic:
                return {type: 'basic', basic: fields({username: a.username, password: a.password})};
            case AuthKind.Bearer:
                return {type: 'bearer', bearer: fields({token: a.token})};
            case AuthKind.ApiKey:
                return {type: 'apikey', apikey: fields({key: a.key, value: a.value, in: a.location})};
        }
    };
    const events = (s?: RequestScripts) => !s?.enabled ? [] : [{
        listen: 'prerequest',
    ...
    const item = collection.requests.map(r => {
        ...
        } : [BodyKind.Form, BodyKind.Multipart].includes(r.body.kind) ? (() => {
            const mode = r.body.kind === BodyKind.Form ? 'urlencoded' : 'formdata';
            return {
                mode,
                [mode]: r.body.fields.map(p => ({
                    key: p.name,
                    value: p.value,
                    disabled: !p.enabled,
                    type: p.file ? 'file' : 'text',
                    src: p.file,
        ...
        return {
            name: r.name,
            event: events(r.scripts),
            request: {
                method: r.method,
                url: {
                    raw: queryUrl(r.url, r.params),
                    query: r.params.map(p => ({key: p.name, value: p.value, disabled: !p.enabled}))
                },
                auth: auth(r.auth),
                header: headers.map(h => ({key: h.name, value: h.value, disabled: !h.enabled})),
                body
            }
        };
    });
