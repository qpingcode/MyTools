...
export function validateWorkspace(value: unknown): asserts value is Workspace {
    ...
    for (const c of v.collections) {
        if (!object(c) || !identifier(c.id) || !text(c.name) || !Array.isArray(c.requests) || c.headers !== undefined && !pairs(c.headers) || c.auth !== undefined && !auth(c.auth, false) || !scripts(c.scripts) || c.settings !== undefined && !settings(c.settings)) throw new Error('Invalid collection');
