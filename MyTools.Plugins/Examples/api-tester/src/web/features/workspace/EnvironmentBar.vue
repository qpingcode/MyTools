  ...
  <div class="environment-bar">
    ...
    <details ref="selector" class="menu environment-selector" @toggle="resetEnvironmentSearch">
      <summary :aria-label="t.Environments()" :title="environment?.name ?? t.NoEnvironment()">
        <span>{{ environment?.name ?? t.NoEnvironment() }}</span>
        <Icon name="chevron-down"/>
      </summary>
      <div class="menu-popover environment-options" :aria-label="t.Environments()">
        <input class="environment-search" v-model="environmentSearch" type="search"
               :placeholder="t.SearchEnvironments()" :aria-label="t.SearchEnvironments()"/>
        <div class="environment-option-list">
          <button :aria-pressed="!workspace.environmentId" @click="selectEnvironment('')">{{
              t.NoEnvironment()
            }}
          </button>
          <button v-for="owner in filteredEnvironments" :key="owner.id"
                  :aria-pressed="workspace.environmentId === owner.id" :title="owner.name"
                  @click="selectEnvironment(owner.id)">{{ owner.name }}
          </button>
          <p v-if="environmentSearch.trim() && !filteredEnvironments.length" class="environment-no-matches"
             role="status">{{ t.NoMatchingEnvironments() }}</p>
        </div>
    ...
    <details ref="menu" class="menu">
      ...
      <div class="menu-popover" @click="dismissAction">
        ...
        >
        <button @click="clearCookies">{{ t.ClearCookies() }}
        </button
        >
