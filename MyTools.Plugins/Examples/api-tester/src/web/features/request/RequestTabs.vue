...
function navigateMenu(event: KeyboardEvent) {
  ...
  if (event.key === 'Tab') { menuOpen.value = false; return; }
...
<template>
  <div ref="tabBar" class="document-tabs-bar">
    <div class="document-tabs">
      <div v-for="item in visibleTabs" :key="item.request.id" class="document-tab" :class="{ selected: item.request.id === active }">
    ...
    <div v-if="hiddenTabs.length" ref="overflowTrigger" class="request-tabs-overflow">
      <IconButton icon="chevron-down" :label="t.HiddenRequests()" aria-haspopup="menu" :aria-expanded="menuOpen" @click="toggleMenu" @keydown.down.prevent="toggleMenu" />
    </div>
    <div v-if="menuOpen && hiddenTabs.length" ref="overflowMenu" class="request-tabs-menu" role="menu" :aria-label="t.HiddenRequests()" @keydown="navigateMenu">
