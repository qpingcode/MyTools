...
function add() {
  tab.value?.request.assertions.push({
    name: '',
    enabled: true,
...
<template>
  <div v-if="tab" class="config-content">
    ...
    <div v-for="(assertion, index) in tab.request.assertions" :key="index" class="assertion-editor-row">
      ...
      <label class="field">{{ t.AssertionKind() }}<select v-model="assertion.kind">
        <option v-for="[kind, label] in assertionOptions" :key="kind" :value="kind">{{ label }}</option>
      </select></label>
      <label
          v-if="assertion.kind !== AssertionKind.Status && assertion.kind !== AssertionKind.Time && assertion.kind !== AssertionKind.Text"
          class="field">{{ t.Path() }}<input v-model="assertion.path"/></label>
      <label v-if="assertion.kind !== AssertionKind.Exists && assertion.kind !== AssertionKind.Header"
             class="field">{{ t.Expected() }}<input v-model="assertion.expected"/></label>
