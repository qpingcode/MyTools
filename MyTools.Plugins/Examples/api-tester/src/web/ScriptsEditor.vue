...
function update(key: keyof RequestScripts, value: string | boolean) { emit('update:modelValue', { ...scripts.value, [key]: value }); }
</script>
<template>
  <div class="config-content scripts-editor">
    <label class="check"><input type="checkbox" :checked="scripts.enabled" @change="update('enabled', ($event.target as HTMLInputElement).checked)" />{{ t.EnableScripts() }}</label>
    <div class="body-toolbar">
      <button :class="{ selected: phase === ScriptPhase.Before }" @click="phase = ScriptPhase.Before">{{ t.BeforeScript() }}</button>
      <button :class="{ selected: phase === ScriptPhase.After }" @click="phase = ScriptPhase.After">{{ t.AfterScript() }}</button>
    </div>
    <textarea class="script-code" :value="scripts[phase]" :aria-label="phase === ScriptPhase.Before ? t.BeforeScript() : t.AfterScript()" spellcheck="false" @input="update(phase, ($event.target as HTMLTextAreaElement).value)" />
    <p class="muted">{{ t.ScriptsHint() }}</p>
    <details><summary>{{ t.ScriptExamples() }}</summary>
