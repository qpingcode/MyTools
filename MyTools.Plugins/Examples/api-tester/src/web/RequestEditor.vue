...
<template>
  <div v-if="tab" class="request-editor">
    ...
    <div class="config-bar">
      <div class="config-tabs" role="tablist" :aria-label="t.Settings()">
        <button
        ...
        >
          {{ t[panel]()
          }}<span v-if="count(panel)" class="tab-count">{{
            count(panel)
          }}</span>
        </button>
      </div>
    ...
    >
      ...
      />
      <PairTable v-if="current === 'Headers'" :values="tab.request.headers" />
      <AuthenticationEditor v-if="current === 'Authentication'" /><BodyEditor
        v-if="current === 'Body'"
      /><AssertionsEditor v-if="current === 'Assertions'" /><ExtractionsEditor
