...
import { BodyKind, Limits, ScriptPhase, ScriptLogLevel, type ApiRequest, type AssertionResult, type RequestResult, type ScriptLog } from '../../shared/model.js';

export interface ScriptOutput { request: ApiRequest; variables: Record<string, string>; logs: ScriptLog[]; tests: AssertionResult[]; error?: string }
export class ScriptFailure extends Error {
  constructor(message: string, public output?: ScriptOutput) { super(message); }
}
...
function bootstrap(inputJson: string, source: string): string | Promise<string> {
  ...
  const logs: any[] = []; const tests: any[] = [];
  const stringify = (v: any) => typeof v === 'string' ? v : JSON.stringify(v);
  const log = (level: string, values: any[]) => { if (logs.length < input.logLimit) logs.push({ phase: input.phase, level, text: values.map(stringify).join(' ').slice(0, input.logChars) }); };
  const variableApi = {
    get: (name: string) => variables[name],
    set: (name: string, value: any) => { variables[String(name)] = String(value); },
    unset: (name: string) => { delete variables[name]; },
  ...
  const list = (pairs: any[]) => ({
    ...
    upsert: (p: any) => { const name = String(p.key ?? p.name); const old = pairs.find(p => p.name.toLowerCase() === name.toLowerCase()); if (old) { old.value = String(p.value); old.enabled = true; } else pairs.push({ name, value: String(p.value), enabled: true }); },
    remove: (name: string) => { for (let i = pairs.length - 1; i >= 0; i--) if (pairs[i].name.toLowerCase() === String(name).toLowerCase()) pairs.splice(i, 1); },
    toObject: () => Object.fromEntries(pairs.filter(p => p.enabled).map(p => [p.name, p.value])),
  });
  function expect(actual: any): any {
    let negate = false; let deep = false;
    ...
    const check = (passed: boolean, expected: any) => { if (negate ? passed : !passed) throw new Error('Expected ' + stringify(actual) + (negate ? ' not ' : ' ') + stringify(expected)); negate = false; deep = false; return chain; };
    const chain: any = {
      ...
      property: (name: string, ...value: any[]) => { check(actual != null && Object.hasOwn(actual, name) && (!value.length || actual[name] === value[0]), name); return expect(actual[name]); },
    ...
    for (const [name, value] of Object.entries({ true: true, false: false, null: null, undefined: undefined })) Object.defineProperty(chain, name, { get: () => check(actual === value, value) });
  ...
  const pm: any = {
    ...
    request: {
      get method() { return request.method; }, set method(v: string) { request.method = String(v); },
      get url() { return request.url; }, set url(v: string) { request.url = String(v); request.params = []; },
      headers: list(request.headers),
      body: { get raw() { return request.body.text; }, set raw(v: string) { request.body.text = String(v); if (request.body.kind === input.bodyNone) request.body.kind = input.bodyText; }, update: (v: any) => { request.body.text = String(typeof v === 'string' ? v : v.raw); if (request.body.kind === input.bodyNone) request.body.kind = input.bodyText; } },
    },
    ...
    test: (name: string, callback: () => any) => {
      ...
      try { if (callback.constructor.name === 'AsyncFunction') throw new Error('Test callbacks must be synchronous'); const result = callback(); if (result && typeof result.then === 'function') { result.catch(() => {}); throw new Error('Test callbacks must be synchronous'); } }
      catch (error: any) { test.passed = false; test.detail = String(error.message || error).slice(0, input.logChars); }
      tests.push(test);
    },
  };
...
export async function runScript(source: string, phase: ScriptPhase, request: ApiRequest, variables: Record<string, string>, signal: AbortSignal, response?: RequestResult, responseText = ''): Promise<ScriptOutput> {
  ...
  return new Promise((resolve, reject) => {
    const worker = new Worker(WorkerSource, {
      ...
      workerData: { script: '', bootstrap: bootstrapSource, timeout: Limits.scriptTimeoutMs, input: { request, variables, phase, response: scriptResponse, responseText, logLimit: Limits.scriptLogEntries, logChars: Limits.scriptLogChars, testLimit: Limits.scriptTests, millisecondsPerSecond: MillisecondsPerSecond, randomIntRange: RandomIntRange, bodyNone: BodyKind.None, bodyText: BodyKind.Text, logLevels: Object.values(ScriptLogLevel) } },
    });
    let settled = false;
    ...
    worker.on('exit', () => { if (!settled) finish(new Error('Script worker exited')); });
    worker.on('message', message => {
      if (!message.ok) { finish(new Error(message.error)); return; }
      try { if (typeof message.value !== 'string' || Buffer.byteLength(message.value) > Limits.scriptOutputBytes) throw new Error('Script output too large'); const output: ScriptOutput = JSON.parse(message.value); if (output.error) finish(new ScriptFailure(output.error, output)); else finish(undefined, output); }
