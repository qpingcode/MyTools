...
界面采用左右工作区：在左侧集合树新增请求；请求列表通过右键菜单或 Shift+F10 上移、下移、复制和删除，集合菜单还可直接运行集合或添加请求。请求标签页右键菜单支持新建、复制、分组关闭及在侧栏中显示。右键菜单支持方向键导航和 Escape 关闭。分隔线可拖动，也支持方向键调整、Home/End 和双击复位。URL 下方通过标签切换参数、请求头、认证、请求体、脚本和设置；响应区域显示正文、响应头和脚本测试结果。请求体标签行可打开仅包含当前请求的历史记录。历史搜索覆盖全部匹配记录，并在前端按每页 5 条分页；在请求历史中清空只删除当前请求的记录，全局 History 中的清空才会删除全部记录。集合设置可以覆盖超时、重定向和 TLS 验证默认值。集合运行从集合右键菜单启动，Runner 会按普通文档标签加入标签顺序；配置页可选择请求、迭代次数、延迟、JSON/CSV 测试数据及高级行为，启动后按测试状态、错误或控制台输出筛选实时结果。关闭 Runner 标签页只会隐藏任务，只有“停止运行”才会取消任务。
...
创建并选择环境，编辑变量，通过 `{{baseUrl}}` 引用。登录关联可在响应后脚本中将 `pm.response.json().token` 保存为 `token`，后续 Bearer 认证引用 `{{token}}`。手动标签共享临时变量，切换环境或重启后清空；每次批量运行使用独立临时变量及 Cookie，结束后丢弃。

在响应后脚本中使用 `pm.test` 和 `pm.expect` 添加测试。勾选部分请求，或不勾选以运行整个当前集合。已打开标签使用当前编辑，不自动保存。支持失败后继续/停止及取消。HTTP 错误码仍是有效响应；没有测试的请求标记为未测试。
...
支持导出整个工作区、单个集合或单个环境；集合也可导出为 Postman v2.1。原生 JSON 保留脚本和请求设置；Postman 导出不包含 API Tester 请求设置。导出使用已保存的数据，请先保存标签页编辑。
...
测试和响应值提取统一写在“响应后”脚本中。单次发送和集合运行产生的 `pm.test` 结果都显示在响应的“测试结果”页签中，Runner 还会汇总通过与失败数量。
...
请求前脚本异常会阻止发送；响应后脚本异常保留收到的响应，并标记运行失败。只有执行、解码及脚本均成功时才提交变量修改；测试失败会将请求标记为失败，并由集合 Runner 汇总。
