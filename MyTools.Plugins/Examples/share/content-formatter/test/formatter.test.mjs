...
import {detectLanguage, formatSource, isLanguageId, tryFormatSource} from "../dist/index.js";
...
for (const [language, source] of cases) {
  test(`detectLanguage recognizes ${language}`, () => assert.equal(detectLanguage(source), language));
...
test("formatSource rejects invalid input", async () => {
  await assert.rejects(() => formatSource("{invalid", "json"));
  await assert.rejects(() => formatSource("<root>", "xml"));
});

test("tryFormatSource preserves unsupported and invalid content", async () => {
  assert.equal(await tryFormatSource("hello world"), "hello world");
  assert.equal(await tryFormatSource("{invalid", "json"), "{invalid");
});
