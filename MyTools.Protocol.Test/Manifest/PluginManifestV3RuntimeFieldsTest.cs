...
public class PluginManifestV3RuntimeFieldsTest
{
    ...
    public void Deserialize_ShouldReadCommandRunnerConfiguration()
    {
        ...
        Assert.That(
            ...
            Is.EquivalentTo(new[] { "name", "runAsAdmin" }));
        Assert.That(
            schema.Properties.Single(property => property.Key == "command").Visibility,
            Is.EqualTo("${isBashScript == false}"));
        Assert.That(
            schema.Properties.Single(property => property.Key == "args").Visibility,
            Is.EqualTo("${isBashScript == false}"));
        Assert.That(
            schema.Properties.Single(property => property.Key == "scripts").Visibility,
