...
- `type`：`string` / `bool` / `int` / `double` / `array` / `path`。`uiHint` 可选：string 默认 `input`（也可 `textarea` / `email` / `telephone`）；bool 默认 `checkbox`（也可 `radio` / `select`）；int/double 默认 `input-number`；array 默认 `table`，必须带 `schema.properties`。`path` 默认 `fileOrDirectory`（也可 `file` / `directory`），Settings 显示文件/目录选择器。列 `type: "hidden"` 表格和编辑框都不显示（时间戳等）。列 `"table": false` 只在编辑对话框出现，不出现在表格。默认值支持宏 `${DateTime.Now}`（新增行时解析）。`visibility` 可选，条件宏，用当前插件其他 configuration 的 `key` 写成表达式，例如 `"visibility": "${ChromeEnabled == true}"`。支持 `==` / `!=` / `&&` / `||` / 括号；条件为真时才在 Settings 里显示该项。省略则始终显示。
